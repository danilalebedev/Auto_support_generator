...
from ...mnova_clipboard import mnova_clipboard_placeholder_map, paste_mnova_clipboard_pages
...
def postprocess_word_objects_node(state: GenerateSIState) -> dict:
    ...
    if request.input_kind == "word":
        paste_word_structures(
            ...
            output_path,
            compounds,
            main_top_offset_pt=12,
            appendix_top_offset_pt=0,
        )
    ...
    if state.get("spectra_config", {}).get("insert_spectra_as", "png") == "mnova":
        mnova_map = mnova_clipboard_placeholder_map(compounds)
        if mnova_map:
            paste_mnova_clipboard_pages(
                output_path,
                mnova_map,
                mnova_exe=request.mnova_exe,
                log_dir=request.output_dir / "logs" / "mnova_clipboard",
            )
