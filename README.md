...
`png` вставляет изображения спектров, `mnova` вставляет изображения страниц,
которые MestReNova уже экспортировала из обработанного `.mnova` через
`draw.toPixmap(page, 300)`, `none` не добавляет спектральный appendix. Для
`mnova`-режима у каждого спектра должны быть сохраненный `.mnova` файл и
сгенерированная page PNG.
