...
`png` вставляет изображения спектров, `mnova` открывает обработанный `.mnova`,
копирует нужную страницу через MestReNova clipboard action и вставляет ее в Word
так же, как при ручном `Ctrl+C` / `Ctrl+V`, `none` не добавляет спектральный
appendix. Для `mnova`-режима у каждого спектра должен быть сохраненный `.mnova`
файл, а на машине должны быть доступны MestReNova и Microsoft Word.
