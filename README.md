...
Journal profile - это высокий уровень настроек под журнал или группу журналов.
Он управляет общим форматом SI: порядком секций, названием профиля, путем к
Word-шаблону и правилами для ссылок/NMR/HRMS. Это необязательное поле: если
оставить его пустым, будет использован стандартный профиль. Более точные
настройки жирного/курсива, индексов, символа `J` и отдельных блоков задаются
через `style_config.yml`.

Сейчас доступны встроенные профили:
...
`png` вставляет изображения спектров, `mnova` вставляет обработанный `.mnova`
как Word OLE-объект, `both` оставляет оба представления, `none` не добавляет
спектральный appendix. Если на конкретной машине Word/MestReNova не смогли
создать OLE preview, программа вставит кликабельную PNG-превью с ссылкой на
соответствующий `.mnova`, чтобы в документе не оставались служебные маркеры.
