...
`png` вставляет изображения спектров, `mnova` вставляет такие же изображения
спектров, но делает их кликабельными ссылками на обработанные `.mnova` файлы,
`none` не добавляет спектральный appendix. Для `mnova`-режима у каждого спектра
должны быть и PNG preview, и сохраненный `.mnova` файл.
