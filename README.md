...
как кликабельный Word OLE-объект, `none` не добавляет спектральный appendix.
Если выбран `mnova`, на машине должен быть зарегистрирован OLE-сервер
MestReNova (`MestReNova.Document.1`). Если Word/MestReNova не смогут создать
OLE-объект, генерация остановится с ошибкой и диагностикой в `logs`.
