...
1. Скачайте `AutoSupportGeneratorSetup.exe` из GitHub Releases или получите файл
   от разработчика.
...
```text
%LOCALAPPDATA%\AutoSupportGenerator\examples\word_workflow\test_input.docx
%LOCALAPPDATA%\AutoSupportGenerator\examples\word_workflow\test_input.zip
%LOCALAPPDATA%\AutoSupportGenerator\examples\word_workflow\example_output\support_information.docx
%LOCALAPPDATA%\AutoSupportGenerator\examples\word_workflow\example_output\processed_spectra.zip
```
...
Если ошибка возникает при обработке спектров, проверьте путь к MestReNova:
